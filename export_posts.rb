...
outfile = File.open('./output/posts.csv', "wb")
outfile << (["Post#", "Topic Id", "Body", "Creation Date [yyyy-MM-dd HH:mm:ss z]", "Submitter"].join(', '))
outfile << "\n"
...
csv_filenames.each do |csv_filename|
  ...
  CSV.foreach(csv_filename, :headers=>true) do |row|
    ...
    next if row["Post#"].nil? || row["Post#"].empty?
...
# puts t
      ...
      [row["Post#"], row["Topic Id"], row["Body"], t, row["Submitter"]].each do |element|
